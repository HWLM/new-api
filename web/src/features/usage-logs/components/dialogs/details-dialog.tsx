...
function shouldSubtractCacheTokens(other: LogOtherData): boolean {
  return other.claude !== true;
}

function getDisplayInputTokens(log: UsageLog, other: LogOtherData): number {
  const sourceInputTokens = other.input_tokens_total ?? log.prompt_tokens ?? 0;
  const inputTokens = Math.max(sourceInputTokens, 0);
  const cacheRead = other.cache_tokens ?? 0;
  if (!shouldSubtractCacheTokens(other) || cacheRead <= 0) return inputTokens;
  return Math.max(inputTokens - cacheRead, 0);
}
...
function TokenBreakdown(props: { log: UsageLog; other: LogOtherData }) {
  ...
  const promptTokens = getDisplayInputTokens(log, other);
  ...
  const hasTokens =
    ...
    cacheWrite5m > 0 ||
    cacheWrite1h > 0 ||
    (other.image && (other.image_output ?? 0) > 0);
