...
// MatrixUnmatchedTier is emitted by visual v2 price matrices when no
// configured cell matches the request. It is a fail-closed control marker,
// not a billable tier.
const MatrixUnmatchedTier = "__matrix_unmatched__"

// IsUnmatchedMatrixTier recognizes both the current marker and the legacy
// zero-cost fallback generated by older visual editors. Explicitly named free
// tiers remain valid.
func IsUnmatchedMatrixTier(version int, matchedTier string, cost float64) bool {
	if version != 2 || cost != 0 {
		return false
	}
	return matchedTier == MatrixUnmatchedTier || matchedTier == "fallback"
}
