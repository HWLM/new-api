...
func modelPriceHelperTieredPerCall(c *gin.Context, info *relaycommon.RelayInfo, groupRatioInfo hosttypes.GroupRatioInfo) (hosttypes.PriceData, error) {
	...
	if rawCost < 0 {
		return hosttypes.PriceData{}, fmt.Errorf("model %s tiered expr produced negative cost %f", info.OriginModelName, rawCost)
	}
	version := billingexpr.ExprVersion(exprStr)
	if billingexpr.IsUnmatchedMatrixTier(version, trace.MatchedTier, trace.Cost) {
		return hosttypes.PriceData{}, fmt.Errorf("model %s pricing matrix does not match request parameters", info.OriginModelName)
	}
	frozenRequestMultiplier, _, err := billingexpr.RunExprRequestMultiplierWithRequest(exprStr, params, requestInput)
	if err != nil {
		return hosttypes.PriceData{}, fmt.Errorf("model %s tiered request multiplier failed: %w", info.OriginModelName, err)
	}
	if math.IsNaN(frozenRequestMultiplier) || math.IsInf(frozenRequestMultiplier, 0) || frozenRequestMultiplier < 0 {
		return hosttypes.PriceData{}, fmt.Errorf("model %s tiered request multiplier is invalid: %f", info.OriginModelName, frozenRequestMultiplier)
	}
