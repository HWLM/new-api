...
func TestGetUserModelsExpandsAutoGroupsInConfiguredOrder(t *testing.T) {
	...
	require.NoError(t, setting.UpdateUserUsableGroupsByJSONString(`{"auto":"自动分组","default":"默认分组","vip":"VIP 分组","unavailable":"不可用分组"}`))
	require.NoError(t, ratio_setting.UpdateUserGroupVisibleGroupsByJSONString(`{"default":["auto","vip","default"]}`))
	specialGroups := ratio_setting.GetGroupRatioSetting().GroupSpecialUsableGroup
	specialGroups.Clear()
