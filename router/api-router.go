	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				// 邀请用户统计：卡片对所有用户开放；图表和表格在 handler 内校验商务账号
				selfRoute.GET("/inviter_stats/cards", controller.GetInviterStatCards)
				selfRoute.GET("/inviter_stats/charts", controller.GetInviterCharts)
				selfRoute.GET("/inviter_stats/summary", controller.GetInviterSummary)
				selfRoute.GET("/inviter_stats/daily", controller.GetInviterDaily)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/:id/quota-history", controller.GetUserQuotaHistory)
		...
		dataRoute.GET("/promotion", middleware.AdminAuth(), controller.GetPromotionStats)
